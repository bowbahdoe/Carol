...
public class TestSuite extends JFrame {
    ...
    private void setButtonText(){
        ...
        button1.setText("Drive a Triangle");
    ...
    private void setDescriptions(){
        ...
        description1.setText("Drive a triangle and report coordinates.  Put # cm for each side in A.");
        ...
        description7.setText("Developing tests...");
        description8.setText("Run Exhibit");
        description9.setText("");
        description10.setText("Wander for C seconds. (Put the number of seconds to drive in C.)");
    }
    
    /** Drive in a triangle, with side length A.
     *
     *
     */
    ...
    private void command1(){
        ...
        if(inputIsValid()){
            ...
            driveAct = new DriveToAction(length);
            turnAct = new TurnToAction(-120, false);
            ...
            for(int i = 0; i < 3; i++){
                ...
                outputln("After drive, position is x:" + position[0] + " y: " + position[1] + " theta: " + position[2]);
                ...
                position = rov.highLevelState.getPosition();
                outputln("After turn, position is x:" + position[0] + " y: " + position[1] + " theta: " + position[2]);
            }
        } else
            System.err.println("Input is invalid");
    }
    
    /** A simple leash. broken*/
...
/*        try{
            Thread.sleep(100);
        }catch(java.lang.InterruptedException e){System.err.println("Leash sleep failed");}
 */
        while(!stop){
            range = rov.state.getRange();
            cm = ScanAction.translateScan(range);
            System.err.println("range is " + cm + " cm");
            
            if((cm > 60 )||(cm == 30))
                driveAct = new DriveToAction(0,0,false);
            else if(cm > 30)
                driveAct = new DriveToAction(-5,0,false);
            else
                driveAct = new DriveToAction(5,0,false);
 ...
 */
     ...
     /*       if(rov.refresh()){
                position = rov.state.getPosition();
                hmmDist = rov.state.getDist();
                position = rov.highLevelState.getPosition();
                hmmDist = rov.highLevelState.getDist();
                System.err.println("Current state distance: " + hmmDist);
                System.err.println("position x: " + position[0] + " y: " + position[1] + " theta: " + position[2]);
            } else System.err.println("Rover refresh failed");*/
        }
    }
    
    /** red light green light using getMean and a grb transform */
    ...
    private void command6(){
        ...
        //sample - time lapse
        //java.util.TimerTask tt = rov.timeLapse(0,0, 320, 240, false, rov.getTopLevelDir()+"/TimeLapse/tlpic",imageLabel); //saves images to file
        java.util.TimerTask tt = rov.timeLapse(0,0, 320, 240, false, imageLabel);
        java.util.Timer timer = rov.executeFunctionTimerAtTime(tt,new java.util.Date(rov.getTime()),10, false);
        while(!stop){
            try{
                Thread.sleep(10);
            }catch(java.lang.InterruptedException e){
                rov.stopExecuteFunction(timer);
            }
        }
        rov.stopExecuteFunction(timer);
        //end sample
    ...
    private void command7(){
        //Test various Actions with different parameters
        Action a;
        //analyze rock
        Log.println("AnalyzeRock(uv on, 320 by 240)",true);
        a = new AnalyzeRockAction(true, 320, 240);
        if(!executeAction(a)){Log.println("!!! AnalyzeRockAction failed.",true);}
        //aproach
